...
const authenticateJWT = async (c, next) => {
  ...
  try {
    ...
    c.set('user', decoded); // Attach user info to the context
    await next();
  } catch (err) {
...
app.post('/signup', async (c) => {
  ...
  const token = generateToken({ id: userData.user_id, role: userData.role });
  delete userData.password; // Remove password from the response
    return c.json({ message: 'User created successfully!', token, user: userData });
  } catch (error) {
